...
def get_pipeline(ensemble_size, iterations):
    ...
    for it in range(1, iterations+1):
        ...
        s2 = Stage()
        s2_task_uids = list()
        for instance in range(ensemble_size):
            ...
            if it == 1:
                ...
                t2.copy_input_data += [ '$Pipeline_%s_Stage_%s_Task_%s/CB7G3_run.mdp'%(p.uid, s4.uid, s4_task_uids[instance]), 
                                        '$Pipeline_%s_Stage_%s_Task_%s/CB7G3.gro'%(p.uid, s3.uid, s3_task_uids[instance])]
        ...
        for instance in range(ensemble_size):
            ...
            t3.cores = 20
            # t3.mpi = True
            t3.copy_input_data = ['$Pipeline_%s_Stage_%s_Task_%s/CB7G3.tpr'%(p.uid, s2.uid, s2_task_uids[instance])]
...
if __name__ == '__main__':
    ...
    pipelines_set.add(get_pipeline(ENSEMBLE_SIZE, TOTAL_ITERS))
