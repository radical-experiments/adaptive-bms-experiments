...
def get_weights_and_was_flat(log_file, weight_size):
    """
        summary: uses log files to determine last weights and if the histogram was ever flat
    ...
    wl_average = np.mean(wldelta)
    if wldelta[-1] == wldelta[0]:
        was_flat = 0
    else:
        was_flat = 1

    weights_OLD = weights_temp

    return weights_OLD, was_flat
...
def free_energy_analysis(dir, prev_data, filelist, template, new_mdp_name, run_number, gen_number):
    ...
    # which files to analyze? For now, ignore the 1/3 of the oldest, rounded down
    # 1 generation - ignore none
    # 2 generations - ignore 1, 
    # 3 generations - ignore 1, 
    # 4 generations - ignore 2, 
    maxgen = np.max(generations)
    analysis_cutoff = int(np.floor(maxgen/3.0))
    if (maxgen - analysis_cutoff) < 1:
        analysis_cutoff = 0
    elif (maxgen - analysis_cutoff) > 50:
        analysis_cutoff = maxgen - 50
    ...
    for d in dhdlfiles:
        ...
        run = int(vals[1].replace('run',''))
        gen = int(vals[2].replace('gen',''))
        if gen in analysis_gens and run is int(run_number):
    ...
    # weights_AVERAGE = get_average_weight_data(log_files, weight_size=len(weights))
    ...
    # WL ANALYSIS AND OLD WEIGHTS- added by Travis Jensen
    # ------------------------------------------------------------------------------------------------------------------
    # read the most recent log file for this instance and determine if the WL increment dropped during the run
    # defining the old log file
    log_file_instance = os.path.join(dir, 'CB7G3_run%s_gen%s.log'%(str(run_number), str(gen_number)))
    # log_file_instance = 'CB7G3_run{0}_gen{1}.log'.format(run_number, gen_number)
    weights_OLD, was_flat = get_weights_and_was_flat(log_file_instance, weight_size=len(weights))
    # ------------------------------------------------------------------------------------------------------------------

    # WEIGHTS LOGIC- added by Travis Jensen
    # ------------------------------------------------------------------------------------------------------------------
    if was_flat:
        weights = weights_MBAR
    else:
        weights = weights_OLD
    # ------------------------------------------------------------------------------------------------------------------
