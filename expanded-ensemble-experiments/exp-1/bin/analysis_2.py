...
def unit_time(file_path):
    log_files = glob.glob(os.path.join(file_path, '*.log'))
    maxgens_per_run = np.zeros(16)
    for l in log_files:
        vals = l.split('/')
        file_name = vals[-1]
        vals_2 = file_name.split('_')
        number = int(vals_2[1].replace('.log', ''))
        if len(str(number)) == 4:
            run = int(str(number)[0])
            gen = int(str(number)[1:])
        else:
            run = int(str(number)[:2])
            gen = int(str(number)[2:])

        if gen > maxgens_per_run[run-1]:
            maxgens_per_run[run-1] = gen

    for index in range(0, len(maxgens_per_run)):
        if maxgens_per_run[index] < 1:
            if np.max(maxgens_per_run) > 1:
                maxgens_per_run[index] = float(np.max(maxgens_per_run) - 1.0)

    total_number = np.sum(maxgens_per_run)
    return total_number
...
def free_energy_analysis(dir, prev_data, filelist, template, new_mdp_name, run_number, gen_number):
    ...
    # determining how much data was available and writing to file
    total_data = unit_time(absolute_analysis_dir)
    data_results = open('data_total.txt', 'w')
    data_results.write(str(total_data) + '\n')
    data_results.close()
    # ------------------------------------------------------------------------------------------------------------------
	...
	# delete all files in analysis_1 directory except the results files
	for fl in glob.glob(os.path.join(absolute_analysis_dir, '*.log')):
	    os.remove(fl)
        for fl2 in glob.glob(os.path.join(absolute_analysis_dir, '*.xvg')):
            os.remove(fl2)
